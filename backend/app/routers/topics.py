...
def _translated_languages(session: Session, topic_id: int) -> list[dict]:
    """Languages that have a completed TRANSLATION task for this topic."""
    tasks = session.exec(
        select(Task).where(
            Task.topic_id == topic_id,
            Task.task_type == TaskType.TRANSLATION,
    ...
    for task in tasks:
        if task.language_id:
            lang = session.get(Language, task.language_id)
            if lang:
                result.append(lang.model_dump())
    return result
...
def list_completed(
    ...
    return [
        {
            **_public_topic(t, user.is_admin),
            "breadcrumb": _ancestors(by_id, t),
            "translated_languages": _translated_languages(session, t.id),
        }
        for t in completed
    ]
...
def get_topic_detail(
    ...
    return {
        ...
        "tasks": task_out,
        "translated_languages": _translated_languages(session, topic.id),
    }
...
class YoutubePrivacyUpdate(SQLModel):
    privacy_status: str


@router.patch("/{topic_id}/youtube-privacy")
def update_youtube_privacy(
    topic_id: int,
    payload: YoutubePrivacyUpdate,
    session: Session = Depends(get_db),
    _: str = Depends(require_admin),
):
    """Change the YouTube privacy of a topic's video. Admin only."""
    if payload.privacy_status not in _VALID_PRIVACY:
        raise HTTPException(
            400, f"privacy_status must be one of: {', '.join(sorted(_VALID_PRIVACY))}"
        )
    topic = get_topic_or_404(session, topic_id)
    if topic.video_state != VideoState.COMPLETED or not topic.youtube_video_id:
        raise HTTPException(400, "This topic has no YouTube video")

    try:
        set_video_privacy(topic.youtube_video_id, payload.privacy_status)
    except Exception as exc:  # noqa: BLE001
        raise HTTPException(502, f"YouTube API error: {exc}")

    topic.youtube_privacy_status = payload.privacy_status
    topic.updated_at = datetime.now(timezone.utc)
    session.add(topic)
    session.commit()
    session.refresh(topic)
    return _public_topic(topic, True)
...
def remove_topic_video(
    ...
    try:
        ...
        topic.youtube_privacy_status = None
